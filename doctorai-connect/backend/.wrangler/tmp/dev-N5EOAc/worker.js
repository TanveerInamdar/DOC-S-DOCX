...
__name(encodeSession, "encodeSession");
function decodeSession(raw2) {
  try {
    return raw2 ? JSON.parse(atob(raw2)) : null;
  } catch {
    return null;
  }
}
...
async function requireSession(c) {
  const s = decodeSession(getCookie(c, "session"));
  if (!s) return c.json({ error: "unauthorized" }, 401);
  return s;
}
__name(requireSession, "requireSession");
async function isDoctor(c) {
  const s = decodeSession(getCookie(c, "session"));
  return s && s.role === "doctor" ? s : null;
}
__name(isDoctor, "isDoctor");
...
<p>\u{1F4CA} <strong>Patient Overview:</strong><br>
\u2022 <strong>Age:</strong> ${(/* @__PURE__ */ new Date()).getFullYear() - new Date(patient.dob).getFullYear()} years old<br>
...
<p>\u{1F48A} <strong>Current Medications:</strong><br>
${currentMeds}</p>

<p>\u26A0\uFE0F <strong>Known Allergies:</strong><br>
${knownAllergies}</p>

<p>\u{1F4C8} <strong>Clinical Assessment:</strong><br>
${appointmentCount > 0 ? `Patient demonstrates ${appointmentCount > 3 ? "regular" : "intermittent"} healthcare engagement with ${appointmentCount} documented visits. Current medication regimen appears appropriate for documented conditions. No critical allergies requiring immediate attention noted. Overall clinical trajectory appears stable.` : "Limited medical history available. Recommend comprehensive initial assessment and baseline documentation."}</p>

<p>\u{1F3AF} <strong>Recommendations:</strong><br>
...
<p>\u{1F50D} <strong>Primary Considerations:</strong><br>
...
<p>\u{1F4CB} <strong>Next Steps:</strong><br>
...
<p>\u{1F48A} <strong>Medication Considerations:</strong><br>
...
<p>\u{1F4DA} <strong>Treatment Guidelines:</strong><br>
...
<p>\u{1F504} <strong>Follow-up Planning:</strong><br>
...
<p>\u{1F3AF} <strong>Clinical Decision Support:</strong><br>
...
<p>\u{1F4CA} <strong>Patient Management:</strong><br>
...
<p>\u{1F50D} <strong>Quality Assurance:</strong><br>
...
<p>\u26A0\uFE0F <strong>Important:</strong> This is a demo response. Always use clinical judgment and consult appropriate resources for patient care decisions.</p>`;
      ...
      aiResponse = `<p>\u{1F6A8} <strong>URGENT MEDICAL SITUATION DETECTED</strong></p>
...
<p>\u{1FA7A} <strong>Clinical Support Areas:</strong><br>
...
<p>\u{1F4CB} <strong>How I Can Help:</strong><br>
...
${context ? `<p>\u{1F4CA} <strong>Current Patient Context:</strong> ${context}</p>` : ""}

${history && history.length > 0 ? `<p>\u{1F4AC} <strong>Conversation History:</strong> I can see we've been discussing related topics.</p>` : ""}
...
// ../../../../Users/inamd/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// ../../../../Users/inamd/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-jL1uvH/middleware-insertion-facade.js
...
// ../../../../Users/inamd/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-jL1uvH/middleware-loader.entry.ts
